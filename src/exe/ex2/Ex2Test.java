...
class Ex2Test {
	...
	static final double[] P1 ={2,0,3, -1,0}, P2 = {0.1,0,1, 0.1,3}, EMPTY = {};
	static double[] po1 = {2,2}, po2 = {-3, 0.61, 0.2}, po3 = {2,1,-0.7, -0.02,0.02}, po4 = {-3, 0.61, 0.2};
	static double[] po5 = {5}, po6 = {5, 0, 0}, po7 = {1, 0, 1}, po8 = {2,1,-0.7,-0.02,0.02,0.1,23};
	static double[] po9 = {6, 0.1, -0.2}, po10 = {2, 5}, po11 = {0, -5}, po12 = {0, 1}, po13 = {0, 0, 0};
	static double[] po14 = {1+EPS/2}, po15 = {1}, po16 = {2, 1, -0.7, -0.02, 0.0, 0.02, 0.1, 0, 23, -2.98765};
	...
	double[] po25 = {-9000, 9000}, po26 = {9000, -9000}, po27 = {0, 1};
	double[] po28 = {3.0, -0.3999999999999999, -0.76, -0.02, 0.12000000000000001, 0.7, 23.0, 184.0, -26.88885};
	static double[] po29 = {0, 0.5}, po30 = {1, 2, 3}, po31 = {0, -5}, po32 = {0, 0, 0}, po33 = {0, 0, 1};
	...
	String ts5 = "5", ts6 = "x^2", ts7 = "x^2 +1", ts8 = "+23x^6 +0.1x^5 +0.02x^4 -0.02x^3 -0.7x^2 +1x^1 +2x^0";
	String ts9 = "-0.2x^2 +0.1x+6", ts10 = "5x +2", ts11 = "-5x", ts12 = "x", ts13 = "0", ts14 = "1.0005";
	String ts15 = "1", ts16 = "-2.98765x^9 +23x^8 +0.1x^6 +0.02x^5 -0.02x^3 -0.7x^2 +x +2";
	 ...
	 * tests that getPolynomFromString returns the expected array:
	 */
	...
	@Test
	/**
	 * Tests that f(x) returns the expected value:
	 */
	void testF() {
		...
		assertEquals(2, fx0, EPS);
		assertEquals(4, fx1, EPS);
		assertEquals(6, fx2, EPS);
		assertEquals(fx3, fx4, EPS);
		assertEquals(fx5, fx6, EPS);
		assertEquals(1.496462, fx7, EPS);
		assertEquals(-5.678, Ex2.root_rec(po4, -10, 0, EPS), 0.1);
	}
	...
	@Test
	/**
	 * Tests that p1+p2 == p2+p1 (symmetric):
	 */
	void testAdd2() {
		double[] p12 = Ex2.add(po1, po2);
	...
	public void testEquals() {
		...
		double[][] d2 = {Ex2.ZERO, {1+EPS/2}, {1,2}};
		double[][] xx = {{-2*EPS}, {1+EPS*1.2}, {1,2,EPS/2}};
	...
	@Test
	/**
	 * Tests if sameValue returns an expected value, with a high degree of accuracy:
	 */
	public void testSameValue() {
		double r1 = -0.1528729116, r2 = -2.1804604217, eps = EPS/20;
		assertEquals(r1, Ex2.sameValue(po30, po31, -1, 0, EPS), eps);
		assertEquals(r2, Ex2.sameValue(po30, po31, -3, -1, EPS), eps);
	}
	...
	public void testSameValue2() {
		double x1=-4, x2=0;
		double rs1 = Ex2.sameValue(po1,po2, x1, x2, EPS);
		double rs2 = Ex2.sameValue(po2,po1, x1, x2, EPS);
		assertEquals(rs1,rs2,EPS);
	}
	...
	public void testSameValue3() {
		assertEquals(1,Ex2.sameValue(po25, po26, 0.02, 3.02, EPS), EPS);
	}
	...
	public void testArea4(){
		...
		assertEquals(16, a1);
		assertEquals(16, a2);
		assertEquals(16, a3, EPS);
	}

	@Test
	/**
	 * test that area between complex polynoms is equal to the manual calculation,
	 * to a high degree of accuracy:
	 */
	public void testArea5(){
		double x1 = Ex2.sameValue(po9, po8, -10, 0, EPS);
		double x2 = Ex2.sameValue(po9, po8, 0, 10, EPS);
		double a1 = Ex2.area(po9, po8, x1, x2, 10000);
		assertEquals(5.277132383807607, a1, EPS);
	}
	...
	public void testRoot(){
		assertEquals(-5.687, Ex2.root_rec(po2, -10, 10, EPS), EPS);
		assertEquals(2.637, Ex2.root_rec(po2, 0, 10, EPS), EPS);
	}
	 ...
	 *  if xx.length != yy.length, expect an empty array:
	 *  if two 'x' values are equal, expect an empty array:
	 */
	public void testPolynomFromPoints(){
		double[] a1 = Ex2.PolynomFromPoints(po2, po4);
		for(int i=0; i<3; i++){ assertEquals(po4[i], Ex2.f(a1, po2[i]), EPS); }
		double[] a2 = Ex2.PolynomFromPoints(po2, po6);
		for(int i=0; i<3; i++){ assertEquals(po6[i], Ex2.f(a2, po2[i]), EPS); }
		double[] a3 = Ex2.PolynomFromPoints(po2, po7);
		for(int i=0; i<3; i++){ assertEquals(po7[i], Ex2.f(a3, po2[i]), EPS); }
		double[] a4 = Ex2.PolynomFromPoints(po2, po8);
		for(int i=0; i<3; i++){ assertEquals(0, Ex2.f(a4, po2[i]), EPS); }
		double[] a5 = Ex2.PolynomFromPoints(po6, po9);
		for(int i=0; i<3; i++){ assertEquals(0, Ex2.f(a5, po6[i]), EPS); }
		double[] a6 = Ex2.PolynomFromPoints(po1, po27);
		assertArrayEquals(EMPTY, a6);
		double[] a7 = Ex2.PolynomFromPoints(po10, po11);
		for(int i=0; i<2; i++){ assertEquals(po11[i], Ex2.f(a7, po10[i]), EPS); }
		double[] a8 = Ex2.PolynomFromPoints(po1, po5);
		assertArrayEquals(EMPTY, a8);
		double[] a9 = Ex2.PolynomFromPoints(po2, po32);
		for(int i=0; i<3; i++){ assertEquals(po32[i], Ex2.f(a9, po2[i]), EPS); }
		assertTrue(Ex2.equals(a9, Ex2.ZERO));
	}
	...
	public void testLength(){
		...
		double a15 = Ex2.length(po33, 0, Math.E, 200);
		double a16 = Ex2.length(po33, 0, Math.E*-1, 200);
		assertEquals(19.9, a0, EPS);
		assertEquals(38.5, a14, EPS);
		assertNotEquals(a1, a2);
		assertEquals(a3, a4, EPS);
		assertEquals(a5, a6, EPS);
		assertEquals(a5, Math.sqrt(2), EPS);
		assertNotEquals(a7, a8);
		assertEquals(a9, a10, EPS);
		assertNotEquals(a11, a12);
		assertEquals(a13, a12, EPS);
